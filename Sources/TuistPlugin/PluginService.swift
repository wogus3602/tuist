...
public final class PluginService: PluginServicing {
    ...
    public func loadPlugins(using config: Config) throws -> Plugins {
        ...
        try zip(pluginPaths, config.plugins).forEach { pluginPath, pluginLocation in
            switch pluginLocation {
...
//                let packageInfo = try
                ...
                let tasksZip = try FileHandler.shared.temporaryDirectory().appending(component: "tasks.zip")
                try System.shared.run("curl", "-L", "--output", tasksZip.pathString, url + "/releases/download/\(tag)/tasks.zip")
